...
for(my $i=0;$i<1000;$i++) {
	...
	foreach(keys(%{$reaanno})) {
		...
		defined($numcomplex->{$fbacodes->{$_}->[0]})?
		($numcomplex->{$fbacodes->{$_}->[0]}++):
		($numcomplex->{$fbacodes->{$_}->[0]}=1);
 		if(@{$fbacodes->{$_}}==2) {
 			$fbasetup->{TFSET}->[$fbamap->{$fbacodes->{$_}->[1]}]->[3]=undef;
 			defined($numcomplex->{$fbacodes->{$_}->[1]})?
			($numcomplex->{$fbacodes->{$_}->[1]}++):
			($numcomplex->{$fbacodes->{$_}->[1]}=1);
 		}
	}
	if($tobin->fbasetupUpdate($ARGV[1],undef,$fbasetup->{TFSET},undef)) {
 		die("Problem updating database.");
 	}
 	my $fbares=runfba($ARGV[1],$ARGV[0]);
...
#	print($genehash->{"b1210"}->[0]."\n");
 	...
 	do {
		...
		$lastrem=$todel->[$rem];
 		...
 		pop(@{$todel});
# 		print(LOG $genelist->[$lastrem]);
 		$geneactive->{$genelist->[$lastrem]}=0;
 		foreach (@{$genehash->{$genelist->[$lastrem]}}) {
 			if(($reaactive->{$_}&&!recompreact($_,$reaanno,$geneactive))) {
 				$reaactive->{$_}=0;
 				if($numcomplex->{$fbacodes->{$_}->[0]}==1) {
 					$fbasetup->{TFSET}->[$fbamap->{$fbacodes->{$_}->[0]}]->[3]=0;
 					$numcomplex->{$fbacodes->{$_}->[0]}=0;
 				}
 				else {
 					$numcomplex->{$fbacodes->{$_}->[0]}--;
 				}
 				if(@{$fbacodes->{$_}}==2) {
 					if($numcomplex->{$fbacodes->{$_}->[1]}==1) {
 						$fbasetup->{TFSET}->[$fbamap->{$fbacodes->{$_}->[1]}]->[3]=0;
 						$numcomplex->{$fbacodes->{$_}->[1]}=0;
 					}
 					else{
 						$numcomplex->{$fbacodes->{$_}->[1]}--;
 					}
...
# 			print(LOG "\tnacc");
 		...
 		elsif($fbares>$lastflux) {
 			die("Higher result, gene deleted: ".$genelist->[$lastrem]);
 		}
 		else {
 #			$lastflux=$fbares->[0];
 #			$lastflux= $fbares;
...
# 		print(LOG "\n");
 		if(!($gcount%100)&&$towrite) {
 			open(WY, ">>ra.".$ARGV[2].".".sprintf("%3d",$gcount).".out");
 			...
 			print(WY "\n");
			close(WY);
			open(WY, ">>ga.".$ARGV[2].".".sprintf("%3d",$gcount).".out");
