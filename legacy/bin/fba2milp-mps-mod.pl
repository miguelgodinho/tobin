...
foreach(@tab) {
	chomp;
	$unblockhash->{$_}=1;
	defined($revhash->{$_})&&($unblockhash->{$revhash->{$_}}=1);
}
#$blockhash->{$ARGV[2]}=1;
...
foreach(keys(%{$limhash})) {
	...
	if(defined($revhash->{$_})&&defined($unblockhash->{$_})) {
		defined($limhash->{$_}->{min})||($limhash->{$_}->{min}=-$limval);
		defined($limhash->{$_}->{max})||($limhash->{$_}->{max}=$limval);
	}
	elsif(defined($unblockhash->{$_})&&(!defined($limhash->{$_}->{max}))) {
		$limhash->{$_}->{max}=$limval;
	}
}
foreach(keys(%{$limhash})) {
...
foreach(keys(%{$limhash})) {
	...
	defined($unblockhash->{$_}))&&print(" L  LAIR".sprintf("%04d",$_)."\n");
	defined($limhash->{$_}->{max})&&(defined($blockhash->{$_})||
	defined($unblockhash->{$_}))&&print(" L  LAAR".sprintf("%04d",$_)."\n");
...
foreach(keys(%{$limhash})) {
	if(defined($limhash->{$_}->{min})&&$limhash->{$_}->{min}!=0) {
		...
		"-1.000000000"."   ");
		if(defined($blockhash->{$_})||defined($unblockhash->{$_})) {
#			print("MAR".sprintf("%04d",$_)."   "."-1000.000000   ");
			print("MAR".sprintf("%04d",$_)."   "."-1.000000000   ");
		}
...
foreach(keys(%{$blockhash})) {
	...
	sprintf("%04d",$_)."   -1000.000000   "."DLIM      1.0000000000   ");
	print("\n    ACTR".sprintf("%04d",$_)."  LAAR".sprintf("%04d",$_).
	"  -1000.000000   ");
	defined($revhash->{$_})&&print("LAIR".sprintf("%04d",$_)."  -1000.000000   "); 	
}
foreach(keys(%{$unblockhash})) {
	defined($skip->{$_})&&next;
	print("\n    ACTR".sprintf("%04d",$_)."  MAR".
	sprintf("%04d",$_)."   -1000.000000   "."  LAAR".sprintf("%04d",$_).
	"  -1000.000000   ");
	defined($revhash->{$_})&&print("\n    ACTR".sprintf("%04d",$_)."  LAIR".
	sprintf("%04d",$_)."  -1000.000000   "); 	
}
