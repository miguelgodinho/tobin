...
foreach(@rlist) {
 ...
 my $fbasetup=$tobin->fbasetupGet(33);
 my $fbamap={};
...
for(my $i=0;$i<50;$i++) {
	...
	if($tobin->fbasetupUpdate(33,undef,$fbasetup->{TFSET},undef)) {
 		die("Problem updating database.");
 	}
 	my $fbares=runfba(33,1008);
	...
	open(WY,">>fbalist2.out");
	foreach(@{$fbalist}) {
		print(WY $_."\t");
	}
	print(WY "\n");
	close(WY);
	open(WY,">>reaactive2.out");
