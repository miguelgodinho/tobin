...
foreach(@{$fba->{TFSET}}) {
	if($_->[1]) {
		defined($skip->{$_->[0]})?($objective->{$revhash->{$_->[0]}}=-1):
		($objective->{$_->[0]}=1);
	}
...
foreach(keys(%{$tfhash})) {
	...
	if(defined($objective->{$_})) {
		($count%2)||print("\n    R".sprintf("%05d",$_)."    ");
		print("OBJ        ".
		(($objective*($fba->{TYPE}==2?1:-1))<0?"-1.000000000":"1.0000000000"));
		$count++;
	}
}
...
foreach(keys(%{$blockhash})) {
	...
	length($numst)<12?($numst.=sprintf("%0*d",(12-length($numst)),"0")):
	($numst=substr($numst,0,12));
	print("\n    ACTR".sprintf("%05d",$_)."  LAAR".sprintf("%05d",$_)."  ".$numst."   ");
	defined($revhash->{$_})&&print("  LAIR".sprintf("%05d",$_)."  ".$numst."   "); 	
}
