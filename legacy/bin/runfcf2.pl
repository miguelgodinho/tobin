...
foreach(@{$fbaset->{TFSET}}) {
	my $tf=$_->[0];
	defined($excluded->{$tf})&&next;
	$tflist->{$tf}=1;
}
my $blocked={};
my $lpdata=getfbaset($fbaset,$tobin);
print(keys(%{$lpdata->{cpdhash}->{1}})."\n");
print(keys(%{$lpdata->{cpdhash}->{0}})."\n");
exit;
foreach(keys(%{$tflist})) {
	my $res=0;
	my $str=$_."\t";
	...
	if(defined($revhash->{$_})) {
		defined($lpcopy->{tfhash}->{$revhash->{$_}})||($lpcopy->{tfhash}->{$revhash->{$_}}={});
	...
	for(my $i=-1;$i<8;$i++) {
		...
		`echo "$file"|~/Software/lp_solve/lp_solve -timeout 30 -S1 -s$i`;
...
for(my $i=0;$i<(@{$tftab}-1);$i++) {
	...
	for(my $j=$i+1;$j<@{$tftab};$j++) {
		...
		$lpcopy->{tfhash}->{$tftab->[$j]}->{min}=10;
		$lpcopy->{tfhash}->{$tftab->[$j]}->{max}=10;
		if(defined($revhash->{$tftab->[$i]})) {
			defined($lpcopy->{tfhash}->{$revhash->{$tftab->[$i]}})||($lpcopy->{tfhash}->{$revhash->{$tftab->[$i]}}={});
			$lpcopy->{tfhash}->{$revhash->{$tftab->[$i]}}->{min}=0;
			$lpcopy->{tfhash}->{$revhash->{$tftab->[$i]}}->{max}=0;
		}
		if(defined($revhash->{$tftab->[$j]})) {
			defined($lpcopy->{tfhash}->{$revhash->{$tftab->[$j]}})||($lpcopy->{tfhash}->{$revhash->{$tftab->[$j]}}={});
			$lpcopy->{tfhash}->{$revhash->{$tftab->[$j]}}->{min}=0;
			$lpcopy->{tfhash}->{$revhash->{$tftab->[$j]}}->{max}=0;
		}
		...
		my $min;
		for(my $k=-1;$k<8;$k++) {	
			my @result=$k<0?`echo "$file"|~/Software/lp_solve/lp_solve -S1 -timeout 30`:
			`echo "$file"|~/Software/lp_solve/lp_solve -timeout 30 -S1 -s$k`;
			if(@result>1&&$result[1]=~/Value of objective function: (.*)$/) {
				$min=$1;
				last;
			}
